...
async function askVersion(check: RushCheck): Promise<DependencyVersion> {
    ...
    let version = await question(
        `Which version to use (${chalk.green(left.version)}/${chalk.yellow(right.version)})? (${chalk.green(
            "[left]"
...
function groupByDependency(dependencyVersions: DependencyVersion[]): Dependencies {
...
function updateVersions(dependencies: Dependencies, packageJson: PackageJson): PackageJson {
    return _.mapValues(packageJson, (value, packageJsonKey) => {
        if (["dependencies", "devDependencies", "peerDependencies"].includes(packageJsonKey)) {
            const dependencyRecord = value;
            return _.mapValues(dependencyRecord, (version: string, dependencyName: string) => {
