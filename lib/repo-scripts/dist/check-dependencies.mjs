...
import require$$0$4, { promises } from 'fs';
...
Object.defineProperty(path$l, "__esModule", { value: true });
path$l.convertPosixPathToPattern = path$l.convertWindowsPathToPattern = path$l.convertPathToPattern = path$l.escapePosixPath = path$l.escapeWindowsPath = path$l.escape = path$l.removeLeadingDotSegment = path$l.makeAbsolute = path$l.unixify = void 0;
const os = require$$0;
...
path$l.unixify = unixify;
function makeAbsolute(cwd, filepath) {
    return path$k.resolve(cwd, filepath);
}
path$l.makeAbsolute = makeAbsolute;
...
path$l.removeLeadingDotSegment = removeLeadingDotSegment;
path$l.escape = IS_WINDOWS_PLATFORM ? escapeWindowsPath : escapePosixPath;
function escapeWindowsPath(pattern) {
    return pattern.replace(WINDOWS_UNESCAPED_GLOB_SYMBOLS_RE, '\\$2');
}
path$l.escapeWindowsPath = escapeWindowsPath;
function escapePosixPath(pattern) {
    return pattern.replace(POSIX_UNESCAPED_GLOB_SYMBOLS_RE, '\\$2');
}
path$l.escapePosixPath = escapePosixPath;
path$l.convertPathToPattern = IS_WINDOWS_PLATFORM ? convertWindowsPathToPattern : convertPosixPathToPattern;
function convertWindowsPathToPattern(filepath) {
    return escapeWindowsPath(filepath)
        .replace(DOS_DEVICE_PATH_RE, '//$1')
        .replace(WINDOWS_BACKSLASHES_RE, '/');
}
path$l.convertWindowsPathToPattern = convertWindowsPathToPattern;
function convertPosixPathToPattern(filepath) {
    return escapePosixPath(filepath);
}
path$l.convertPosixPathToPattern = convertPosixPathToPattern;
...
var stringify$8 = (ast, options = {}) => {
  const stringify = (node, parent = {}) => {
    const invalidBlock = options.escapeInvalid && utils$k.isInvalidBrace(parent);
...
const compile$1 = (ast, options = {}) => {
  const walk = (node, parent = {}) => {
    const invalidBlock = utils$j.isInvalidBrace(parent);
    const invalidNode = node.invalid === true && options.escapeInvalid === true;
    ...
    if (node.nodes && node.ranges > 0) {
      const args = utils$j.reduce(node.nodes);
...
const expand$1 = (ast, options = {}) => {
  ...
  const walk = (node, parent = {}) => {
    ...
    const enclose = utils$i.encloseBrace(node);
...
picomatch$2.matchBase = (input, glob, options, posix = utils$e.isWindows(options)) => {
  ...
  return regex.test(path$i.basename(input));
};
...
function isAffectDepthOfReadingPattern(pattern) {
    const basename = path$h.basename(pattern);
...
function processPatterns(input, settings) {
    ...
    if (settings.braceExpansion) {
        patterns = utils$c.pattern.expandPatternsWithBraceExpansion(patterns);
    }
    /**
     ...
     */
    return patterns.map((pattern) => utils$c.pattern.removeDuplicateSlashes(pattern));
}
/**
...
class EntryFilter {
    ...
    getFilter(positive, negative) {
        ...
        const negativeRe = utils$5.pattern.convertPatternsToRe(negative, Object.assign(Object.assign({}, this._micromatchOptions), { dot: true }));
    ...
    _isMatchToPatterns(filepath, patternsRe, isDirectory) {
        ...
        const isMatched = utils$5.pattern.matchAny(filepath, patternsRe);
...
(function (FastGlob) {
    ...
    function convertPathToPattern(source) {
        assertPatternsInput$1(source);
        return utils$2.path.convertPathToPattern(source);
    }
    FastGlob.convertPathToPattern = convertPathToPattern;
    (function (posix) {
        function escapePath(source) {
            assertPatternsInput$1(source);
            return utils$2.path.escapePosixPath(source);
        }
        posix.escapePath = escapePath;
        function convertPathToPattern(source) {
            assertPatternsInput$1(source);
            return utils$2.path.convertPosixPathToPattern(source);
        }
    ...
    (function (win32) {
        function escapePath(source) {
            assertPatternsInput$1(source);
            return utils$2.path.escapeWindowsPath(source);
        }
        win32.escapePath = escapePath;
        function convertPathToPattern(source) {
            assertPatternsInput$1(source);
            return utils$2.path.convertWindowsPathToPattern(source);
        }
...
function isTypeSync(fsStatType, statsMethodName, filePath) {
	...
	try {
		return require$$0$4[fsStatType](filePath)[statsMethodName]();
	} catch (error) {
		if (error.code === 'ENOENT') {
			return false;
		}
...
isType.bind(null, 'stat', 'isFile');
const isDirectory = isType.bind(null, 'stat', 'isDirectory');
isType.bind(null, 'lstat', 'isSymbolicLink');
isTypeSync.bind(null, 'statSync', 'isFile');
isTypeSync.bind(null, 'statSync', 'isDirectory');
isTypeSync.bind(null, 'lstatSync', 'isSymbolicLink');
...
checkPath$1.isNotRelative = isNotRelative;
...
const isPathValid = path =>
  checkPath$1(path && checkPath$1.convert(path), path, RETURN_FALSE);
...
if (
  ...
  checkPath$1.convert = makePosix;

  // 'C:\\foo'     <- 'C:\\foo' has been converted to 'C:/'
  // 'd:\\foo'
  const REGIX_IS_WINDOWS_PATH_ABSOLUTE = /^[a-z]:\//i;
  checkPath$1.isNotRelative = path =>
...
const applyBaseToPattern = (pattern, base) => isNegativePattern(pattern)
	? '!' + path$m.posix.join(base, pattern.slice(1))
	: path$m.posix.join(base, pattern);

const parseIgnoreFile = (file, cwd) => {
	const base = slash(path$m.relative(cwd, path$m.dirname(file.filePath)));
...
const toRelativePath = (fileOrDirectory, cwd) => {
	cwd = slash(cwd);
	if (path$m.isAbsolute(fileOrDirectory)) {
		if (slash(fileOrDirectory).startsWith(cwd)) {
			return path$m.relative(cwd, fileOrDirectory);
		}
...
const normalizePathForDirectoryGlob = (filePath, cwd) => {
	...
	return path$m.isAbsolute(path) ? path : path$m.join(cwd, path);
};

const getDirectoryGlob = ({directoryPath, files, extensions}) => {
	const extensionGlob = extensions?.length > 0 ? `.${extensions.length > 1 ? `{${extensions.join(',')}}` : extensions[0]}` : '';
	return files
		? files.map(file => path$m.posix.join(directoryPath, `**/${path$m.extname(file) ? file : `${file}${extensionGlob}`}`))
		: [path$m.posix.join(directoryPath, `**${extensionGlob ? `/*${extensionGlob}` : ''}`)];
};
...
const createFilterFunction = isIgnored => {
	...
	return fastGlobResult => {
		const pathKey = path$m.normalize(fastGlobResult.path ?? fastGlobResult);
...
async function run() {
    ...
    const dependencies = await Promise.all(packages.map(async (file) => ({
    ...
    results.forEach(({ project, items }) => {
        console.log(`Project: ${chalk.bold(project)} has declared following as dependency instead of devDependency:`);
