...
async function askVersion(check) {
    ...
    let version = await question(`Which version to use (${chalk.green(left.version)}/${chalk.yellow(right.version)})? (${chalk.green("[left]")}/${chalk.yellow("(r)ight")}/(s)kip/somethingelse): `);
...
function groupByDependency(dependencyVersions) {
    return _.chain(dependencyVersions)
        .groupBy("dependencyName")
        .mapValues((items) => _.head(items))
        .value();
}
function updateVersions(dependencies, packageJson) {
    return _.mapValues(packageJson, (value, packageJsonKey) => {
        if (["dependencies", "devDependencies", "peerDependencies"].includes(packageJsonKey)) {
            ...
            return _.mapValues(dependencyRecord, (version, dependencyName) => {
...
async function main() {
    const versionsJson = await $ `rush check --json`.json();
    const rushJson = await $ `rush list --json`.json();
    const dependencies = await index
        .mapSeries(versionsJson.mismatchedVersions, askVersion)
        .then((deps) => deps.filter((d) => d.version !== "skip"))
        .then(groupByDependency);
    await index.eachSeries(rushJson.projects, 
