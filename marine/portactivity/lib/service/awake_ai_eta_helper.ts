...
export function destinationIsFinnish(locode: string | undefined): boolean {
...
export function predictionToTimestamp(
    ...
    if (prediction.predictionType !== AwakeAiPredictionType.ETA) {
        console.warn(
            "method=AwakeAiETAHelper.handleSchedule state=%s, IMO: %d, LOCODE: %s, portcallid: %d",
    ...
    if (!destinationIsFinnish(prediction.locode)) {
        console.warn(
            "method=AwakeAiETAHelper.predictionToTimestamp state=%s, IMO: %d, LOCODE: %s, portcallid: %d",
            AwakeDataState.PREDICTED_DESTINATION_OUTSIDE_FINLAND,
            imo,
            locode,
            portcallId
        );
    ...
    if (!prediction.recordTime) {
        console.warn(
            "method=AwakeAiETAHelper.handleSchedule state=%s, using current time, IMO: %d, LOCODE: %s, portcallid: %d",
    ...
    console.info(
        "method=AwakeAiETAHelper.predictionToTimestamp prediction was valid, IMO: %d, LOCODE: %s, portcallid: %d",
        imo,
        locode,
        portcallId
    );
    ...
    console.info(
        "method=AwakeAiETAHelper.predictionToTimestamp created timestamp: %s",
