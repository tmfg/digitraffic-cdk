...
describe('schedules', () => {
    ...
    test('schedulesToTimestamps - under VTS control - [x] ETA [ ] ETD', async () => {
        const timestamps = schedulesToTimestamps(createSchedulesResponse(3, true, false), false);

        expect(timestamps.length).toBe(3);
        timestamps.forEach(ts => verifyStructure(ts, EventType.ETA, false));
    });

    test('schedulesToTimestamps - under VTS control - [ ] ETA [x] ETD', async () => {
        const timestamps = schedulesToTimestamps(createSchedulesResponse(3, false, true), false);

        expect(timestamps.length).toBe(3);
        timestamps.forEach(ts => verifyStructure(ts, EventType.ETD, false));
    });

    test('schedulesToTimestamps - under VTS control - [x] ETA [x] ETD', async () => {
        const timestamps = schedulesToTimestamps(createSchedulesResponse(3, true, true), false);
    ...
    test('schedulesToTimestamps - calculated - [x] ETA [ ] ETD', async () => {
        const timestamps = schedulesToTimestamps(createSchedulesResponse(3, true, false), true);

        expect(timestamps.length).toBe(3);
        timestamps.forEach(ts => verifyStructure(ts, EventType.ETA, true));
    });

    test('schedulesToTimestamps - calculated - [ ] ETA [x] ETD', async () => {
        const timestamps = schedulesToTimestamps(createSchedulesResponse(3, false, true), true);

        expect(timestamps.length).toBe(3);
        timestamps.forEach(ts => verifyStructure(ts, EventType.ETD, true));
    });

    test('schedulesToTimestamps - calculated - [x] ETA [x] ETD', async () => {
        const timestamps = schedulesToTimestamps(createSchedulesResponse(3, true, true), true);
