...
describe("event-sourceutil", () => {
    ...
    test("mergeTimestamps - timestamps are sorted after merge", () => {
        ...
        const vtsTimestamp = newTimestamp({ eventTime: vtsTime.toDate(), source: EventSource.SCHEDULES_CALCULATED, portcallId });
        const portnetTimestamp = newTimestamp({ eventTime: portnetTime.toDate(), source: EventSource.PORTNET, portcallId });
        const vtsControlTimestamp = newTimestamp({ eventTime: vtsCTime.toDate(), source: EventSource.SCHEDULES_VTS_CONTROL, portcallId });
    ...
    test("mergeTimestamps - picks highest priority source", () => {
        const portcallId = 1;
        const schedulesTimestamp = newTimestamp({ source: EventSource.SCHEDULES_VTS_CONTROL, portcallId });
