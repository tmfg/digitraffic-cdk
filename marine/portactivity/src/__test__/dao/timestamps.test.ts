...
describe(
  ...
  dbTestBase((db: DTDatabase) => {
    ...
    test("removeTimestamps - found 1", async () => {
      ...
      // wrong id
      const notRemoved = await TimestampsDb.removeTimestamps(db, EVENT_SOURCE, [
        "2",
      ]);
    ...
    function testNewest(
      ...
      test(`${description} - multiple - only newest`, async () => {
        ...
        expect(foundTimestamp.record_time.toISOString()).toBe(
          timestamp2.recordTime,
        );
      });
    }
    ...
    testMissingPortCallIdWithPastEventtime(
      "findByMmsi",
    ...
    test("findPortnetETAsByLocodes - 23 h in future is found", async () => {
      ...
      const foundTimestamps = await TimestampsDb.findPortnetETAsByLocodes(db, [
        locode,
      ]);
    ...
    test("findVtsShipImosTooCloseToPortByPortCallId - returns ships closer than specified", async () => {
      ...
      const eventTime = addMinutes(
        new Date(),
        shipApproachThresholdMinutes - 1,
      );
