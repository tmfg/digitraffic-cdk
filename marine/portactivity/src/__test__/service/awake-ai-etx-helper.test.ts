...
describe("Awake.AI ETA helper", () => {
  ...
  test("isDigitrafficEtaPrediction - correct", () => {
    ...
    expect(AwakeAiETAHelper.isDigitrafficEtaPrediction(digitrafficEta)).toBe(
      true,
    );
  ...
  test("isPortcallPrediction - correct", () => {
    ...
    const awakePortCallPredictionInvalidUrn = newETAPrediction({
      ...
      portCallUrn:
        "urn:awake:some-other-portcall:1234567" as AwakeDigitrafficPortCallURN,
    });
    ...
    expect(isPortcallPrediction(awakePortCallPredictionInvalidUrn)).toEqual(
      false,
    );
