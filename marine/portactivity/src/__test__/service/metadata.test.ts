...
describe(
    ...
    dbTestBase((db: DTDatabase) => {
        test("findLocodesWithPredictions", async () => {
            const predictedTimestamps = PREDICTION_SOURCES.map((source) => newTimestamp({ source }));
            ...
            predictedTimestamps.forEach((timestamp) => expect(locodes).toContain(timestamp.location.port));
        });
    })
);
