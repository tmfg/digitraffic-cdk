...
describe(
  ...
  dbTestBase((db: DTDatabase) => {
    ...
    test("findAllTimestamps - VTS A timestamps are merged", async () => {
      ...
      expect(parseISO(foundTimestamp.eventTime).valueOf()).toBeGreaterThan(
        earlier.valueOf(),
      );
      expect(parseISO(foundTimestamp.eventTime).valueOf()).toBeLessThan(
        later.valueOf(),
      );
      // recordtime should be from higher priority source
      expect(parseISO(foundTimestamp.recordTime).valueOf()).toEqual(
        later.valueOf(),
      );
    });
