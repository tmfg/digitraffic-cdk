...
function datesDifferByMinutes(
  date1: Date,
  date2: Date,
  maxDiffMinutes: number,
): boolean {
  const diffMinutes = differenceInMinutes(date1, date2, {
    roundingMethod: "ceil",
  });
...
export function mergeTimestamps(ts: unknown[]): unknown[] {
  ...
  const filteredTimestamps =
    filterDuplicateAwakeAiPredTimestampsWithoutPortcallId(
  ...
  for (const portcallTimestamps of byPortcallId) {
    ...
    if (vtsAStamps.length > 1) {
      ...
      const highestPriority = vtsAStamps.sort(
        (a, b) =>
          (eventSourcePriorities.get(b.source) ?? 0) -
          (eventSourcePriorities.get(a.source) ?? 0),
      )[0];

      addToList.push({
        ...highestPriority,
        ...{
          eventTime: dateAverage(
            vtsAStamps.map((ts) => parseISO(ts.eventTime)),
          ),
        },
