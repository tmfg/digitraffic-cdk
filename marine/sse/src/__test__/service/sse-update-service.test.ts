...
describe(
  ...
  DbTestutil.dbTestBase((db: DTDatabase) => {
    test("save report", async () => {
      ...
      const sseReportsFromDbBefore: SseDb.DbSseReport[] = await DbTestutil
        .findAllSseReports(db);
      const sseReport: SSESchema.TheSSEReportRootSchema = Testdata
        .createSampleData([Testdata.site1]);
      const savedCount = await SseUpdateService.saveSseData(
        sseReport.SSE_Reports,
      );
      expect(savedCount.saved).toBe(1);

      const sseReportsFromDb: SseDb.DbSseReport[] = await DbTestutil
        .findAllSseReports(db);
