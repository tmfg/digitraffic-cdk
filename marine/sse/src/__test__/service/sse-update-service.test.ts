...
describe(
    ...
    DbTestutil.dbTestBase((db: DTDatabase) => {
        test("save report", async () => {
            ...
            const sseReportsFromDbBefore: SseDb.DbSseReport[] =
                await DbTestutil.findAllSseReports(db);
            const sseReport: SSESchema.TheSSEReportRootSchema =
                Testdata.createSampleData([Testdata.site1]);
            const savedCount = await SseUpdateService.saveSseData(
                sseReport.SSE_Reports
            );
            expect(savedCount.saved).toBe(1);

            const sseReportsFromDb: SseDb.DbSseReport[] =
                await DbTestutil.findAllSseReports(db);
            expect(sseReportsFromDb.length).toBe(
                sseReportsFromDbBefore.length + 1
            );
            ...
            const end = new Date();
            expect(
                lastUpdated && start <= lastUpdated && lastUpdated <= end
            ).toBe(true);
        });
        ...
        test("replace latest report", async () => {
            ...
            const sseReportsFromDb: SseDb.DbSseReport[] =
                await DbTestutil.findAllSseReports(
                    db,
        ...
        test("save multiple reports for same site", async () => {
            ...
            const sseReportsFromDb: SseDb.DbSseReport[] =
                await DbTestutil.findAllSseReports(db);
            expect(sseReportsFromDb.length).toBe(
                sseReportsFromDbBefore.length + 5
            );
            const lastUpdated = await LastUpdatedDB.getUpdatedTimestamp(
                db,
                SseUpdateService.SSE_DATA_DATA_TYPE
