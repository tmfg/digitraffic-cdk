...
describe(
  ...
  DbTestutil.dbTestBase((db: DTDatabase) => {
    ...
    test("update sse report latest status", async () => {
      ...
      const allAfter2ndUpdate: SseDb.DbSseReport[] = await DbTestutil
        .findAllSseReports(db);
      expect(allAfter2ndUpdate.length).toBe(4);
      const site1ReportsAfter2ndUpdate: SseDb.DbSseReport[] = allAfter2ndUpdate
        .filter(
          (r) => r.siteNumber === siteNum1,
        );
      const site2Reports2ndUpdate: SseDb.DbSseReport[] = allAfter2ndUpdate
        .filter(
