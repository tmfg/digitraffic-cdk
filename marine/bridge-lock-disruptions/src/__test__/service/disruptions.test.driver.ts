...
export class DisruptionsTestDriver {
  ...
  public async assertTimestampsCheckedAndUpdated(): Promise<void> {
    ...
    expect(this.timestamps[lastIndex - 1]!.updated).toBeLessThan(
      this.timestamps[lastIndex]!.updated,
    );
    expect(this.timestamps[lastIndex - 1]!.updated).toBeLessThan(
      this.timestamps[lastIndex]!.checked,
    );
  }
  ...
  public async assertTimestampsCheckedNotUpdated(): Promise<void> {
    ...
    expect(this.timestamps.length).toBeGreaterThan(1);
    expect(this.timestamps[lastIndex - 1]!.updated).toEqual(
      this.timestamps[lastIndex]!.updated,
    );
    expect(this.timestamps[lastIndex - 1]!.checked).toBeLessThan(
      this.timestamps[lastIndex]!.checked,
    );
  }
  ...
  public static createRandomDisruptions(
    amount: number = 10,
  ): SpatialDisruption[] {
    return Array.from({
      length: amount,
