...
export async function getUpdatedTimestamps(
  db: DTDatabase,
): Promise<UpdatedTimestamps> {
  const updated = await LastUpdatedDB.getUpdatedTimestamp(
    db,
    BRIDGE_LOCK_DISRUPTIONS_DATA_TYPE,
  );
  const checked = await LastUpdatedDB.getUpdatedTimestamp(
    db,
    BRIDGE_LOCK_DISRUPTIONS_CHECK,
  );
