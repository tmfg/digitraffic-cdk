...
const TEST_ACTIVE_WARNINGS_VALID: FeatureCollection = {
...
describe(
    ...
    dbTestBase((db: DTDatabase) => {
        ...
        test("getActiveWarnings - value", async () => {
            ...
            const warnings = await getActiveWarnings();
            expect(warnings).toEqual(TEST_ACTIVE_WARNINGS_VALID);
        });
        ...
        test("getArchivedWarnings - value", async () => {
            await insertArchivedWarnings(db, TEST_ARCHIVED_WARNINGS_VALID);
            const warnings = await getArchivedWarnings();
            expect(warnings).toEqual(TEST_ARCHIVED_WARNINGS_VALID);
        });
        ...
        test("updateNauticalWarnings - valid geojson", async () => {
            ...
            sinon
                ...
                .returns(Promise.resolve(TEST_ARCHIVED_WARNINGS_VALID));
