...
export class PublicApi {
    ...
    createResources(publicApi: DigitrafficRestApi): void {0
        ...
        const featureModel = addServiceModel(
            "FeatureModel",
            publicApi,
            featureSchema(getModelReference(warningModel.modelId, publicApi.restApiId))
        );
    ...
    createEndpoint(stack: DigitrafficStack): void {
        ...
        ["GET", "HEAD"].forEach((httpMethod) => {
            this.activeResource.addMethod(httpMethod, activeIntegration, {
                apiKeyRequired: true,
                methodResponses: [
                    DigitrafficMethodResponse.response200(this.geojsonModel, MediaType.APPLICATION_GEOJSON),
