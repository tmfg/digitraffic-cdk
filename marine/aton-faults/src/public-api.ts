...
export function create(stack: DigitrafficStack): DigitrafficRestApi {
    ...
    publicApi.documentResource(
        faultsResource,
        DocumentationPart.method(ATON_FAULT_TAGS_V1, "GetActiveFaults", "Return all active faults in GeoJSON")
    );
    publicApi.documentResource(
        faultsResource,
        DocumentationPart.queryParameter("language", "Language: en, fi or sv (default en)")
    );
    publicApi.documentResource(
        faultsResource,
...
function createFaultsResource(stack: DigitrafficStack, publicApi: DigitrafficRestApi, faultsJsonModel: Model): Resource {
    ...
    ["GET", "HEAD"].forEach((httpMethod) => {
        faultsResource.addMethod(httpMethod, getFaultsIntegration, {
            ...
            methodResponses: [
                DigitrafficMethodResponse.response200(faultsJsonModel, MediaType.APPLICATION_GEOJSON),
