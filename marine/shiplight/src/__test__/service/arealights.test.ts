...
describe("arealights service", () => {
    test("updateLightsForArea - calls API", async () => {
        ...
        const updateLightsForAreaStub = jest.spyOn(AreaLightsApi.prototype, "updateLightsForArea").mockResolvedValue({
                LightsSetSentFailed: [],
    ...
    test("updateLightsForArea - retry on error", async () => {
        ...
        const areaTraffic = createAreaTraffic();
        const updateLightsForAreaStub = jest.spyOn(AreaLightsApi.prototype, "updateLightsForArea")
        ...
        expect(updateLightsForAreaStub).toHaveBeenNthCalledWith(1, expectedArguments);
        expect(updateLightsForAreaStub).toHaveBeenNthCalledWith(2, expectedArguments);
    });
