...
export async function upsertVisits(db: DTDatabase, response: NemoResponse): Promise<DbInsertedUpdated> {
    const method = `VisitsDAO.upsertVisits` satisfies LoggerMethodType;

    return db
        .tx<DbInsertedUpdated>((tx) => {
            return tx
                .batch<DbInsertedUpdated>(response.map((visit) => upsertVisit(tx, visit)))
