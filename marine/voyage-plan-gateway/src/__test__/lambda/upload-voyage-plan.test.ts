...
describe("upload-voyage-plan", () => {
  ...
  async function expectEvent(
    ...
    const { handler } = await import(
      "../../lambda/upload-voyage-plan/lambda-upload-voyage-plan.js"
    );
  ...
  test("validation failure, no waypoints xml", async () => {
    await expectEvent(
      voyagePlanWithoutRoutes(),
      "XML structure validation failed",
    );
  });

  test("validation success with correct voyage plan", async () => {
    jest.spyOn(VtsApi.prototype, "sendVoyagePlan").mockReturnValue(
      Promise.resolve(),
    );
    jest.spyOn(RtzStorageApi.prototype, "storeVoyagePlan").mockReturnValue(
      Promise.resolve(),
    );
...
<route version="1.2" xmlns="http://www.cirm.org/RTZ/1/2">
  ...
  <schedules>
    ...
    <schedule id="200" name="string">
      ...
      <manual>
        <!--1 or more repetitio-->
        <scheduleElement waypointId="200" etd="${
    addMinutes(
      new Date(),
      5,
    ).toISOString()
  }" etdWindowBefore="P3M8DT13H38M37S" etdWindowAfter="P1Y2M8DT15H6M2S" eta="${
    addMinutes(
      new Date(),
      5,
    ).toISOString()
