...
export function handlerFn(
    ...
    return async function (): Promise<void> {
        return await doWithSecret(secretId, async (secret: any) => {
            const privateVisUrl = secret[
                VoyagePlanSecretKeys.PRIVATE_VIS_URL
            ] as string;
            const appId = secret[VoyagePlanSecretKeys.APP_ID];
            const apiKey = secret[VoyagePlanSecretKeys.API_KEY];
            const messages = await VisApi.getMessages(
                privateVisUrl,
                appId,
                apiKey
            );

            const routeMessages = messages.message.filter(
                (msg) => msg.messageType === VisMessageType.RTZ
            );
            // Do these contain failed authentications?
            const txtMessages = messages.message.filter(
                (msg) => msg.messageType === VisMessageType.TXT
            );

            console.info(
                `method=vpgwProcessVisMessages count=${messages.message.length}`
            );

            if (messages.remainingNumberOfMessages > 50) {
                console.warn(
                    "method=vpgwProcessVisMessages More than 50 messages remain in queue count=%d",
            ...
            txtMessages.forEach((msg) =>
                console.info(
                    "method=vpgwProcessVisMessages Received TXT message: %s",
                    msg.stmMessage
                )
            );
            ...
            for (const routeMessage of routeMessages) {
                console.info(
                    "method=vpgwProcessVisMessages Processing RTZ message %s",
                ...
                const gzippedMessage = zlib.gzipSync(
