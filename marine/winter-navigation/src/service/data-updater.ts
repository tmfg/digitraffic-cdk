...
export class DataUpdater {
    ...
    async update(): Promise<void> {
        ...
        await inDatabase(async (db: DTDatabase) => {
            await this.updateObjects(db, "wn_location", to, "location", saveAllLocations)
            await this.updateObjects(db, "wn_restriction", to, "restriction", saveAllRestrictions)
            await this.updateObjects(db, "wn_vessel", to, "vessel", saveAllVessels)
            await this.updateObjects(db, "wn_activity", to, "activity", saveAllActivities)
            await this.updateObjects(db, "wn_source", to, "source", saveAllSources)
            await this.updateObjects(db, "wn_port_suspension", to, "port-suspension", saveAllPortSuspensions);
            await this.updateObjects(db, "wn_port_suspension_location", to, "port-suspension-location", saveAllPortSuspensionLocations)
            ...
            await this.updateObjects(db, "wn_dirwaypoint", to, "dirwaypoint", saveAllDirwaypoints);
        });
    }
