...
function convertDirways(dirways: Dirway[], dirwaypoints: Dirwaypoint[]): DTDirway[] {
    const pointMap = groupBy(dirwaypoints, "dirway_id");

    return dirways.map(d => convertDirway(d, pointMap));
}

function convertDirway(dirway: Dirway, pointMap: Record<string, Dirwaypoint[]>): DTDirway {
