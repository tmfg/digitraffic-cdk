...
import { mockProxyHolder, expectResponse, expectResponseContent, expectNotFound } from "@digitraffic-cdk/testing";
import { saveAllLocations } from "../../db/locations.js";
import { LOCATION_1, PORT_SUSPENSION_1, PORT_SUSPENSION_LOCATION_1, RESTRICTION_1 } from "../service/data-updater.test.js";
import { type DTLocation } from "../../model/dt-apidata.js";
import { type LambdaResponse } from "@digitraffic/common/dist/aws/types/lambda-response";
import { saveAllRestrictions } from "../../db/restrictions.js";
import { saveAllPortSuspensionLocations, saveAllPortSuspensions } from "../../db/port-suspensions.js";
...
async function getResponseFromLambda(event: Record<string, string> = {}): Promise<LambdaResponse> {
    const { handler } = await import("../../lambda/get-locations/get-locations.js");
...
describe(
    ...
    dbTestBase((db: DTDatabase) => {
        ...
        test("get all - one location", async () => {
            ...
            expectResponseContent(response, 200, (locations: DTLocation[]) => {
        ...
        test("get one - found", async () => {
            ...
            expectResponseContent(response, 200, (location: DTLocation) => {
                expect(location.name).toEqual(LOCATION_1.name);
                expect(location.restrictions?.length).toEqual(1);
                expect(location.restrictions![0]!.textCompilation).toEqual(RESTRICTION_1.text_compilation);

                expect(location.suspensions?.length).toEqual(1);
                expect(location.suspensions![0]!.dueTo).toEqual(PORT_SUSPENSION_1.due_to);
            });
        });
