...
export function create(stack: DigitrafficStack): DigitrafficRestApi {
    ...
    const [locationResource, locationsResource] = createLocationResources(stack, v2Resource, locationsModel);

    const vesselsModel = publicApi.addJsonModel("VesselsModel", vesselSchema);
    const [vesselResource, vesselsResource] = createVesselResources(stack, v2Resource, vesselsModel); 

    const dirwaysModel = publicApi.addJsonModel("DirwaysModel", dirwaySchema);
    const dirwaysResource = createDirwayResources(stack, v2Resource, dirwaysModel); 
...
function createLocationResources(stack: DigitrafficStack, v2Resource: Resource, locationsModel: Model): [Resource, Resource] {
    ...
    const getLocationIntegration = new DigitrafficIntegration(getLocationsLambda, MediaType.APPLICATION_JSON)
...
function createVesselResources(stack: DigitrafficStack, v2Resource: Resource, locationsModel: Model): [Resource, Resource] {
    const getVesselsLambda = MonitoredDBFunction.create(stack, "get-vessels", undefined, {
        memorySize: 256,
        reservedConcurrentExecutions: 6
    });
    ...
    const getVesselIntegration = new DigitrafficIntegration(getVesselsLambda, MediaType.APPLICATION_JSON)
        .addPathParameter("vessel-id")
        .build();

    const getVesselsIntegration = new DigitrafficIntegration(getVesselsLambda, MediaType.APPLICATION_JSON)
        .passAllQueryParameters()
        .build();
...
function createDirwayResources(stack: DigitrafficStack, v2Resource: Resource, dirwaysModel: Model): Resource {
    const getDirwaysLambda = MonitoredDBFunction.create(stack, "get-dirways", undefined, {
        memorySize: 256,
        reservedConcurrentExecutions: 6
    });
    ...
    const getDirwaysIntegration = new DigitrafficIntegration(getDirwaysLambda, MediaType.APPLICATION_JSON)
        .passAllQueryParameters()
        .build();
    ...
    ["GET", "HEAD"].forEach((httpMethod) => {
        dirwaysResource.addMethod(httpMethod, getDirwaysIntegration, {
            //            apiKeyRequired: true,
                        methodResponses: [
                            DigitrafficMethodResponse.response200(dirwaysModel, MediaType.APPLICATION_JSON),
                            DigitrafficMethodResponse.response400(),
                            DigitrafficMethodResponse.response500()
                        ]
                    });
