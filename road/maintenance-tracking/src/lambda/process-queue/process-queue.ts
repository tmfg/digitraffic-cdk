...
export const handler: (
  event: SQSEvent,
) => Promise<PromiseSettledResult<void>[]> = handlerFn(sqsExtendedClient);
...
export function handlerFn(
  ...
  return async (event: SQSEvent): Promise<PromiseSettledResult<void>[]> => {
    ...
    const extendedSqsMessage: ReceiveMessageCommandOutput =
      await sqsExtendedClient._processReceive(
    ...
    return Promise.allSettled(
      extendedSqsMessage.Messages.map(async (m) => {
        try {
          if (m.Body) {
            const tyokoneenKirjaus = JSON.parse(
              m.Body,
          ...
          logger.error({
            ...
            customMessageContent: JSON.stringify({
              ...m,
              Body: "{...REMOVED...}",
            }),
