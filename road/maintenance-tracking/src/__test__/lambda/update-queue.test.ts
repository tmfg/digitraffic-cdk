...
const { handlerFn, invalidRequest, ok } = await import(
  "../../lambda/update-queue/update-queue.js"
);
const testEvent =
  (await import("../test-apigw-event.json")) as unknown as APIGatewayEvent;
...
describe("update-queue", () => {
  ...
  test("no records - should reject", async () => {
    ...
    const sendMessageStub = jest.spyOn(sqsClient, "sendMessage")
      .mockReturnValue(await Promise.resolve());

    await expect(() => handlerFn(sqsClient)({ ...testEvent, body: null }))
      .rejects.toMatchObject(
        invalidRequest("Empty message"),
      );
  ...
  test("invalid record", async () => {
    ...
    await expect(() => handlerFn(sqsClient)({ ...testEvent, body: json }))
      .rejects.toMatchObject(
        invalidRequest(
