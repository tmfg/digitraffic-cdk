...
describe(
  ...
  dbTestBase((db: DTDatabase) => {
    ...
    test("no records", async () => {
      ...
      await expect(handlerFn(extendedSqsClient)(event)).rejects.toMatch(
        "SQSEvent records was empty.",
      );
    ...
    test("single valid record", async () => {
      const json = getTrackingJsonWith3Observations(
        getRandompId(),
        getRandompId(),
      );
    ...
    test("two valid records", async () => {
      ...
      const json1 = getTrackingJsonWith3Observations(
        getRandompId(),
        getRandompId(),
      );
      const json2 = getTrackingJsonWith3Observations(
        getRandompId(),
        getRandompId(),
      );
    ...
    test("invalid and valid record", async () => {
      ...
      const event = createSQSEventWithBodies([
        "invalidJsonS2Ref",
        "validJsonS3Ref",
      ]);
      // Output without big message json as it's in S3
