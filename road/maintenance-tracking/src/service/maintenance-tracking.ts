...
export async function saveMaintenanceTrackingObservationData(
  ...
  const a: (DbNumberId | undefined)[] = await inDatabase((db: DTDatabase) => {
    return MaintenanceTrackingDb.insertMaintenanceTrackingObservationData(
      db,
      observationDatas,
    );
  });
...
export function cleanMaintenanceTrackingData(
  ...
  const method =
    `${service}.cleanMaintenanceTrackingData` satisfies LoggerMethodType;
...
export function createMaintenanceTrackingMessageHash(
  maintenanceTrackingDataJson: string,
): string {
  // Strip away viestitunniste that changes between messages. We are interested only from the data around it.
  const data = maintenanceTrackingDataJson.replace(
    matchViestitunnisteRegex,
    "",
  );
