...
export function handlerFn(
    doWithSecret: SecretFunction<MaintenanceTrackingPaikanninSecret>
) {
    return async (): Promise<TrackingSaveResult> => {
        ...
        if (!paikanninUpdateService) {
            console.info(
                `method=MaintenanceTrackingMunicipality.paikanninUpdateData domain=${domainName} lambda was cold`
            );
            await doWithSecret(
                secretId,
                (secret: MaintenanceTrackingPaikanninSecret) => {
                    const paikanninApi = new PaikanninApi(
                        secret.apikey,
                        secret.url
                    );
