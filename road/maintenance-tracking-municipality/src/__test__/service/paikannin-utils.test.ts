...
describe("paikannin-utils-service-test", () => {
  ...
  test("groupEventsToIndividualTrackings - events over 5 minute time limit", () => {
    ...
    const now = new Date();
    const over5Min = sub(now, {
      seconds: PAIKANNIN_MAX_TIME_BETWEEN_TRACKINGS_S + 1,
    });
    const events: ApiWorkevent[] = [
  ...
  test("isOverTimeLimit", () => {
    ...
    const onLimit = add(now, {
      seconds: PAIKANNIN_MAX_TIME_BETWEEN_TRACKINGS_S,
    });
