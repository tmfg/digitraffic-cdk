...
describe(
  ...
  dbTestBase((db: DTDatabase) => {
    ...
    test("resolveNextStartTimeForDataFetchFromHistory over 12 month", () => {
      ...
      const shouldResolveTo = sub(new Date(), {
    ...
    test("updateTasks", async () => {
      ...
      const taskMappings1: DbDomainTaskMapping[] = await inDatabaseReadonly(
        (ro: DTDatabase) => {
    ...
    test("updateContracts", async () => {
      ...
      const contractsWithSouce: DbDomainContract[] = await inDatabaseReadonly(
        (ro: DTDatabase) => {
    ...
    test("updateTrackings invalid linestring", async () => {
      ...
      Asserter.assertToBeCloseTo(
        tracking0.last_point.coordinates[0]!,
        POINT_START[0]!,
        0.000001,
      );
      Asserter.assertToBeCloseTo(
        tracking0.last_point.coordinates[1]!,
        POINT_START[1]!,
        0.000001,
      );
