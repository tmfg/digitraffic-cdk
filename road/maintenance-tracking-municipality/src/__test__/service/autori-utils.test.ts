...
describe("autori-utils-service-test", () => {
  ...
  test("fixApiRouteData", () => {
    ...
    const coords2 = createZigZagCoordinates(
      15,
      AUTORI_MAX_DISTANCE_BETWEEN_TRACKINGS_M - 10,
    );
  ...
  test("groupEventsToIndividualGeometries split when big jump", () => {
    const ls = createLineStringGeometry(
      20,
      AUTORI_MAX_DISTANCE_BETWEEN_TRACKINGS_M - 10,
    );
  ...
  test("isOverTimeLimit", () => {
    const now = new Date();
    const insideLimit = add(now, {
      seconds: AUTORI_MAX_TIME_BETWEEN_TRACKINGS_S - 1,
    });
    const onLimit = add(now, { seconds: AUTORI_MAX_TIME_BETWEEN_TRACKINGS_S });
    const outsideLimit = add(now, {
      seconds: AUTORI_MAX_TIME_BETWEEN_TRACKINGS_S + 1,
    });
  ...
  test("resolveNextStartTimeForDataFromApi in limits", () => {
    const inLimits = add(new Date(), {
      minutes: AUTORI_MAX_MINUTES_TO_HISTORY - 1,
    });
  ...
  test("createDbDomainContracts", () => {
    ...
    const contracts = AutoriUtils.createDbDomainContracts(
      apiContracts,
      DOMAIN_1,
    );
    expect(contracts).toHaveLength(2);
    expect(contracts.find((c) => c.name === CONTRACT_1)?.contract).toEqual(
      apiContracts[0]?.id,
    );
    expect(contracts.find((c) => c.name === CONTRACT_2)?.contract).toEqual(
      apiContracts[1]?.id,
    );
  });

  test("createDbWorkMachine", () => {
    const wm: DbWorkMachine = AutoriUtils.createDbWorkMachine(
      CONTRACT_ID,
      DOMAIN_1,
      VEHICLE_TYPE,
    );
  ...
  test("createDbMaintenanceTracking", () => {
    ...
    const route: ApiRouteData = AutoriTestutils.createApiRouteData(now, [
      geometry,
    ]);
    ...
    expect(tracking?.end_time).toEqual(
      AutoriTestutils.createTrackingEndTimeFromUpdatedTime(now),
    );
  });
