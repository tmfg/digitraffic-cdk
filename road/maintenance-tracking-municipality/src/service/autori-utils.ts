...
function toPositionGroups(
  ...
  if (targetGeometries.length > 0) {
    ...
    const prevLineString: Position[] | undefined =
...
export function resolveNextStartTimeForDataFromApi(
  ...
  let resolvedTime = sub(new Date(), {
    minutes: AUTORI_MAX_MINUTES_TO_HISTORY,
  });
  if (contract.data_last_updated) {
    logger.debug(
      `method=AutoriUpdate.resolveContractLastUpdateTime contract=${contract.contract} and domain=${contract.domain} using contract.data_last_updated ${contract.data_last_updated.toISOString()}`,
    );
    resolvedTime = contract.data_last_updated;
  } else if (contract.start_date) {
    logger.debug(
      `method=AutoriUpdate.resolveContractLastUpdateTime contract=${contract.contract} and domain=${contract.domain} using contract.start_date ${contract.start_date.toISOString()}`,
    );
    resolvedTime = contract.start_date;
  } else {
    logger.debug(
      `method=AutoriUpdate.resolveContractLastUpdateTime contract=${contract.contract} and domain=${contract.domain} using -7, 'minutes' ${resolvedTime.toLocaleString()}`,
    );
  }
...
export function createDbMaintenanceTracking(
  ...
  if (!f) {
    ...
    const lastPos: Position | undefined =
