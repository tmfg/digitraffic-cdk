...
function toEventGroups(
  ...
  if (nextEvent.timestamp.getTime() <= filterBeforeOrEquaTime.getTime()) {
    ...
    const prevGroup: ApiWorkevent[] | undefined =
      targetGroups[targetGroups.length - 1];
    const prevEvent: ApiWorkevent | undefined = prevGroup
  ...
  const lineStringCoordinates: Position[] = events.reduce(
    (coordinates: Position[], nextEvent) => {
      ...
      if (coordinates.length > 0) {
        const previousCoordinate: Position | undefined =
...
export function createDbMaintenanceTracking(
  ...
  const geometry: GeoJsonLineString | GeoJsonPoint | undefined =
