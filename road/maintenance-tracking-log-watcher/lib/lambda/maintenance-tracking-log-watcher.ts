...
export const handler = async (): Promise<void> => {
    const fromISOString = moment()
        .subtract(1, "weeks")
        .startOf("isoWeek")
        .toDate()
        .toISOString();
    const toISOString = moment()
        .subtract(1, "weeks")
        .endOf("isoWeek")
        .toDate()
        .toISOString();
    console.info(
        `method=maintenanceTrackingLogWatcherHandler start from ${fromISOString} to ${toISOString}`
    );
    ...
    try {
        ...
        await esService
            ...
            .then(async function (resultLogLines) {
                if (resultLogLines.length > 0) {
                    ...
                    console.info(
                        `method=maintenanceTrackingLogWatcherHandler Upload file ${fileName} to S3 Bucket  ${s3BucketName}`
                    );
        ...
        console.error(
            `method=maintenanceTrackingLogWatcherHandler Error in execution from ${fromISOString} to ${toISOString} error=${error}`
        );
        return Promise.reject(error);
    } finally {
        console.info(
            `method=maintenanceTrackingLogWatcherHandler from ${fromISOString} to ${toISOString} tookMs=${
                Date.now() - start
            }`
        );
    }
