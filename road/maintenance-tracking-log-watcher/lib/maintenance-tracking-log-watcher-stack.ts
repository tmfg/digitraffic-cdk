...
import { AssetCode, Function as AFunction } from "aws-cdk-lib/aws-lambda";
...
import {
    ...
    KEY_S3_BUCKET_NAME,
    KEY_SNS_TOPIC_ARN,
} from "./lambda/maintenance-tracking-log-watcher";
...
function createLambdaRole(stack: Stack, appProps: Props) {
    const lambdaRole = new Role(
        stack,
        "MaintenanceTrackingLogWatcherStackLambdaElasticSearchRole",
        {
...
function createEmailTopic(stack: Stack, appProps: Props) {
    const emailTopic = new sns.Topic(
        stack,
        "MaintenanceTrackingLogWatcherTopic"
    );
...
function createLambdaTrigger(stack: Stack, lambdaFunction: AFunction) {
...
function createWatchLogAndUploadToS3Lambda(
    ...
    return new AFunction(stack, functionName, lambdaConf);
}
