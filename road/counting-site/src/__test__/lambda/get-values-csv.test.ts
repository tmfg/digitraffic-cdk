...
const EXPECTED_CSV_HEADER = "SITE,TRAVELMODE,TIMESTAMP,GRANULARITY,DIRECTION,COUNT\r\n" as const;

async function getResponseFromLambda(event: Record<string, string> = {}): Promise<LambdaResponse> {
    const { handler } = await import("../../lambda/get-values-csv/get-values-csv.js");

    return await handler(event);
}

describe("get-values-csv-lambda", dbTestBase((db: DTDatabase) => {
    ...
    test("get values - invalid traveltype", async () => {
        const response = await getResponseFromLambda({travelType: "eurofighter"});

        new ExpectResponse(response).expectStatus(400);
    });

    test("get values - with date no data", async () => {
        const response = await getResponseFromLambda({year: "2024", month: "8", siteId: "1"});
    ...
    test("get values - with date - data found", async () => {
        ...
        ExpectResponse.ok(response).expectBody(EXPECTED_CSV_HEADER + EXPECTED_DATA_LINE);
    });
    ...
    test("get values - with date & travelmode - data found", async () => {
        ...
        const response = await getResponseFromLambda({year: "2024", month: "8", siteId: "1", travelMode: "bike"});
    ...
    test("get values - with date & wrong travelmode - no data", async () => {
        ...
        const response = await getResponseFromLambda({year: "2024", month: "8", siteId: "1", travelMode: "car"});
