...
import { findValuesForDate, findCsvValuesForMonth } from "../dao/data.js";
...
export function getSites(siteId?: number, domain?: string): Promise<[FeatureCollection, Date]> {
    return inDatabaseReadonly((db: DTDatabase) => {
        return getSitesAsFeatureCollection(db, siteId, domain).then((featureCollection) => {
...
export function findSiteData(
    ...
    return inDatabaseReadonly(async (db: DTDatabase) => {
        const [data, lastModified] = await findValuesForDate(db, date, siteId, travelMode);
