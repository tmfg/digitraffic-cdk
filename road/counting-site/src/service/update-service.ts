...
import { addSites, getAllSites, removeSites, updateSites, updateSiteTimestamp } from "../dao/site.js";
import type { Domain } from "../model/v2/types.js";
import { addSiteData } from "../dao/data.js";

export async function updateMetadata(url: string, apiKey: string, domain: Domain): Promise<void> {
    ...
    const [newSites, removedSites, updatedSites] = compareSites(sitesInApi, sitesInDb);
...
export async function updateData(url: string, apiKey: string, domain: Domain): Promise<void> {
    const api = new EcoCounterApi(url, apiKey);
