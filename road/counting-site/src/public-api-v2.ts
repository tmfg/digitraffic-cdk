...
import { JsonSchemaType, type Model, type Resource } from "aws-cdk-lib/aws-apigateway";
...
export class PublicApiV2 {
    ...
    createTravelModesEndpoint(v2Resource: Resource): void {
        ...
        this.publicApi.documentResource(
            travelModesResource,
            DocumentationPart.method(COUNTING_SITE_TAGS_V2, "GetTravelModes", "Return all travel modes")
        );
    }
    ...
    createGeoJsonResponseModel(): Model {
        ...
        const featureModel = this.publicApi.addJsonModel(
            "SiteFeatureModel",
            featureSchema(getModelReference(siteModel.modelId, this.publicApi.restApiId))
        );
        ...
        return this.publicApi.addJsonModel("SitesModel", geojsonSchemaWithDataUpdatedTime);
    }
    ...
    createSitesEndpoint(stack: DigitrafficStack, v2Resource: Resource): void {
        ...
        const sitesIntegration = new DigitrafficIntegration(lambda, MediaType.APPLICATION_GEOJSON)
            .passAllQueryParameters()
            .build();

        const siteIntegration = new DigitrafficIntegration(lambda, MediaType.APPLICATION_GEOJSON)
    ...
    createValuesEndpoint(stack: DigitrafficStack, v2Resource: Resource): void {
        ...
        const jsonValuesResponseModel = this.publicApi.addJsonModel("JsonDataResponseModel", valueSchema);
    ...
    createCsvValuesEndpoint(stack: DigitrafficStack, v2Resource: Resource): void {
        ...
        ["GET", "HEAD"].forEach((httpMethod) => {
            csvValuesResource.addMethod(httpMethod, integration, {
                ...
                methodResponses: [
                    DigitrafficMethodResponse.response200(csvValuesResponseModel, MediaType.TEXT_CSV),
                    DigitrafficMethodResponse.response500()
                ]
            });
        });
        ...
        this.publicApi.documentResource(
            csvValuesResource,
            DocumentationPart.method(COUNTING_SITE_TAGS_V2, "GetDataCSV", "Return counter values in CSV."),
            DocumentationPart.queryParameter("siteId", "Site id"),
    ...
    createDirectionsEndpoint(v2Resource: Resource): void {
        ...
        this.publicApi.documentResource(
            directionsResource,
            DocumentationPart.method(COUNTING_SITE_TAGS_V2, "GetDirections", "Return all directions")
        );
    }

    createDomainsEndpoint(v2Resource: Resource): void {
        const domainsResource = v2Resource.addResource("domains");
        const domainsResponseModel = this.publicApi.addJsonModel("DomainsResponseModel", domainsSchema);
        const headers = { "Last-Modified": new Date().toUTCString() };
        ...
        this.publicApi.documentResource(
            domainsResource,
            DocumentationPart.method(COUNTING_SITE_TAGS_V2, "GetDomains", "Return all domains")
        );
    }
}
