...
describe(
    ...
    dbTestBase((db: DTDatabase) => {
        ...
        test("find - found", async () => {
            ...
            const foundServiceRequest = await RequestsDb.find(
        ...
        test("update - modify", async () => {
            ...
            await RequestsDb.update(
                [Object.assign({}, serviceRequest, updatingServiceRequest)],
                db
            );
            const foundServiceRequests = await RequestsDb.findAll(db);
            ...
            expect(foundServiceRequest.status_notes).toBe(
                updatingServiceRequest.status_notes
            );
            expect(foundServiceRequest.service_name).toBe(
                updatingServiceRequest.service_name
            );
            expect(foundServiceRequest.service_code).toBe(
                updatingServiceRequest.service_code
            );
            expect(foundServiceRequest.description).toBe(
                updatingServiceRequest.description
            );
            ...
            expect(foundServiceRequest.agency_responsible).toBe(
                updatingServiceRequest.agency_responsible
            );
            expect(foundServiceRequest.service_notice).toBe(
                updatingServiceRequest.service_notice
            );
            expect(foundServiceRequest.address).toBe(
                updatingServiceRequest.address
            );
            expect(foundServiceRequest.address_id).toBe(
                updatingServiceRequest.address_id
            );
            expect(foundServiceRequest.zipcode).toBe(
                updatingServiceRequest.zipcode
            );
            expect(foundServiceRequest.media_url).toBe(
                updatingServiceRequest.media_url
            );
            expect(foundServiceRequest.status_id).toBe(
                updatingServiceRequest.status_id
            );
            expect(foundServiceRequest.vendor_status).toBe(
                updatingServiceRequest.vendor_status
            );
            expect(foundServiceRequest.title).toBe(
                updatingServiceRequest.title
            );
            expect(foundServiceRequest.service_object_id).toBe(
                updatingServiceRequest.service_object_id
            );
            expect(foundServiceRequest.service_object_type).toBe(
                updatingServiceRequest.service_object_type
            );
            expect(foundServiceRequest.media_urls).toMatchObject(
                updatingServiceRequest.media_urls
            );
            expect(foundServiceRequest.subject_id).toBe(
                updatingServiceRequest.subject_id
            );
            expect(foundServiceRequest.subSubject_id).toBe(
                updatingServiceRequest.subSubject_id
            );
        });
        ...
        test("Delete", async () => {
            ...
            expect(
                foundServiceRequests.find(
                    (sr) => sr.service_request_id === srIdToDelete
                )
            ).toBeUndefined();
        });
    })
