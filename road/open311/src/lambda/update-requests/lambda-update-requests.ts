...
export const handler = async (event: APIGatewayEvent): Promise<ProxyLambdaResponse> => {
    ...
    const obj = JSON.parse(event.body) as unknown as ServiceRequestWithExtensionsDto;
    const serviceRequests = Array.isArray(obj) ? (obj as ServiceRequestWithExtensionsDto[]) : [obj];
