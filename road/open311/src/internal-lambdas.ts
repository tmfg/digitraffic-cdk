...
export function create(stack: DigitrafficStack): void {
  const integrationEnvVars: IntegrationEnv = {
    ENDPOINT_USER: stack.getSecret().secretValueFromJson(
      "open311.integration.username",
    ).unsafeUnwrap(),
    ENDPOINT_PASS: stack
      .getSecret()
      .secretValueFromJson("open311.integration.endpoint_pass")
      .unsafeUnwrap(),
    ENDPOINT_URL: stack.getSecret().secretValueFromJson(
      "open311.integration.endpoint_url",
    ).unsafeUnwrap(),
  };

  const updateServicesLambda = createUpdateServicesLambda(
    integrationEnvVars,
    stack,
  );
  const updateStatesLambda = createUpdateStatesLambda(
    integrationEnvVars,
    stack,
  );
  const updateSubjectsLambda = createUpdateSubjectsLambda(
    integrationEnvVars,
    stack,
  );
  const updateSubSubjectsLambda = createUpdateSubSubjectsLambda(
    integrationEnvVars,
    stack,
  );
...
function createUpdateServicesLambda(
  ...
  const updateServicesLambda = MonitoredDBFunction.create(
    stack,
    updateServicesId,
    integrationEnvVars,
  );
...
function createUpdateStatesLambda(
  ...
  const updateStatesLambda = MonitoredDBFunction.create(
    stack,
    updateStatesId,
    integrationEnvVars,
  );
...
function createUpdateSubjectsLambda(
  ...
  const updateSubjectsLambda = MonitoredDBFunction.create(
    stack,
    updateSubjectsId,
    integrationEnvVars,
  );
