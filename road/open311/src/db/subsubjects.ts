...
const INSERT_SUBSUBJECT_PS = new PreparedStatement({
  ...
  text:
    `INSERT INTO open311_subsubject(active, id, locale, name, subject_id) VALUES ($1, $2, $3, $4, $5)`,
});

const SELECT_SUBSUBJECTS_PS = new PreparedStatement({
  name: "select-subsubjects",
  text:
    "SELECT active, id, locale, name, subject_id FROM open311_subsubject WHERE locale = $1 ORDER BY id",
});
