...
export function insertDeviceDataRows(db: DTTransaction, id: number, tt: TloikRivi) {
    return db.none(SQL_INSERT_DEVICE_DATA_ROW, [id, tt.naytto, tt.rivi, tt.teksti]);
}
...
export function insertDeviceData(db: DTTransaction, tt: TloikLiikennemerkinTila) {
    return db.one(SQL_INSERT_DEVICE_DATA, [tt.tunnus, tt.nayttama, tt.lisatieto, tt.voimaan, tt.syy, tt.luotettavuus]).then(r => r.id);
}
