...
const PS_INSERT_DATEX2 = new pgPromise.PreparedStatement({
  ...
  text: `insert into device_data_datex2(device_id,datex2,effect_date,type,version)
           values($1, $2, $3, $4, $5)
...
export function findAll(
  ...
  return db
    .one(PS_FIND_ALL_ACTIVE_LAST_MODIFIED, [version, datexType])
    .then((modified: DbModified) => {
      return db
        .manyOrNone(PS_FIND_ALL_ACTIVE, [version, datexType])
        .then((data: DbSituation[]) => [data, modified.modified]);
    });
}
