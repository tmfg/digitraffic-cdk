...
export function findPermitsInGeojson() {
  return inDatabaseReadonly(async (db) => {
    const geojsonPermits: FeatureCollection = await PermitsDAO
      .getActivePermitsGeojson(db);
...
function convertPermit(permitElement: PermitElement): ApiPermit {
  ...
  // eslint-disable-next-line @typescript-eslint/non-nullable-type-assertion-style
  const permitType = mapLupatyyppi(
    permitObject["GIS:Lupatyyppi_koodi"],
  ) as PermitType;
  ...
  return {
    ...
    effectiveFrom: parse(
      `${permitObject["GIS:VoimassaolonAlkamispaiva"]} ${
        permitObject["GIS:VoimassaolonAlkamisaika"]
      }`,
      "dd.MM.yyyy HH:mm",
      new Date(),
    ),
    effectiveTo:
      ...
      permitObject["GIS:VoimassaolonPaattymispaiva"] != null
        ? parse(
          `${permitObject["GIS:VoimassaolonPaattymispaiva"]} ${
            permitObject["GIS:VoimassaolonPaattymissaika"]
          }`,
          "dd.MM.yyyy HH:mm",
          new Date(),
        )
...
function xmlToJs(xml: string): Promise<PermitResponse> {
  return xml2js.parseStringPromise(xml, { explicitArray: false }) as Promise<
    PermitResponse
  >;
}
