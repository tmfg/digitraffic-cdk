...
const MARINE_ACCOUNT_FILTER_REGEX = new RegExp(
  `^accountName.keyword:${MARINE_ACCOUNT_NAME}`,
);
const ROAD_ACCOUNT_FILTER_REGEX = new RegExp(
  `^accountName.keyword:${ROAD_ACCOUNT_NAME}`,
);
const RAIL_ACCOUNT_FILTER_REGEX = new RegExp(
  `^accountName.keyword:${RAIL_ACCOUNT_NAME}`,
);

export function getTransportTypeDbFilterFromAccountNameFilter(
  filter: string,
): string | undefined {
  if (
...
export function getUriFiltersFromPath(
  path: string,
): { osFilter: OsUriFilter; dbFilter: DbUriFilter } {
  return {
