...
export class EcsMonitoring {    
    ...
    constructor(stack: Stack, alarmsTopic: Topic, ecsConfiguration: EcsConfiguration) {
        this.stack = stack;
    ...
    createAlarms(configuration: EcsConfiguration): void {
        configuration.services.forEach(service => {
            if(service.cpu !== "DISABLED") {
                ...
                createAlarm(this.stack, this.alarmsTopic, cpuAlarmName, cpuMetric, service.cpu ?? DEFAULTS.CPU);
            }
            ...
            if(service.memory !== "DISABLED") {
                ...
                createAlarm(this.stack, this.alarmsTopic, memoryAlarmName, memoryMetric, service.memory ?? DEFAULTS.MEMORY);
            }
        });                    
    }
}