...
def make_wsgi_env(event):
    headers = {'HTTP_{}'.format(name.replace('-', '_').upper()): val
    ...
    env = {
        ...
        'CONTENT_LENGTH': len(body.decode('utf-8')),
...
def handler(event, context):
    ...
    app.logger.debug(f'method=app.handler message=lambda event {event}')
    app.logger.debug(f'method=app.handler message=wsgi env {env}')
    ...
    app.logger.info(f'method=app.handler httpMethod={method} path={path} took={time.time()-start} message={result}')
