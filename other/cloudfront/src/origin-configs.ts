...
function createOriginHeaders(
  ...
  if (domain.cfName !== undefined) {
    ...
    const secret = Secret.fromSecretCompleteArn(
      stack,
      domain.cfName + "-secret",
      secretsArn,
    );
    const cfHeaderName = secret.secretValueFromJson("cfHeaderName").toString();
    const cfHeaderValue = secret.secretValueFromJson("cfHeaderValue")
      .toString();
...
function createBehaviors(
  ...
  return behaviors.map((b) =>
    createBehavior(b, lambdaMap, b.path === "*", isS3Origin)
  );
}
...
function createBehavior(
  ...
  if (isS3Origin && b.path.includes("swagger")) {
    ...
    if (b.queryCacheKeys && b.queryCacheKeys.length > 0) {
      throw new Error(
        "Swagger origin has cache keys!" + JSON.stringify(b.queryCacheKeys),
      );
    }
  }
