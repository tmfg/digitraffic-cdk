    ...
    constructor(scope: Construct, id: string, props: AwsGithubProps, stackProps: StackProps) {
        ...
        props.roles.forEach(role => {
            ...
            if (role.allowedActions.length) {
                createdRole.addToPrincipalPolicy(new PolicyStatement({
                    effect: Effect.ALLOW,
            ...
            if (role.updateSwaggerForTrafficType) {
                ...
                createdRole.addToPrincipalPolicy(new PolicyStatement({
                    ...
                    resources: [
                        Function.fromFunctionName(this, updateApiDocsFunctionName, updateApiDocsFunctionName).functionArn,
                        Function.fromFunctionName(this, updateSwaggerDocsFunctionName, updateSwaggerDocsFunctionName).functionArn,
                    ],
                }));
            }
        });
