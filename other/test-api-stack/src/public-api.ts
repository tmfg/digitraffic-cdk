...
export function create(stack: DigitrafficStack): DigitrafficRestApi {
    const publicApi = new DigitrafficRestApi(stack, "TEST-public", "Test public API");
...
function createTestGetResources(stack: DigitrafficStack, publicApi: DigitrafficRestApi, testModel: Model): void {
    const testGetLambda = MonitoredDBFunction.create(stack, "test-get", undefined, {
        singleLambda: true,
        memorySize: 256,
    ...
    const v1Resource = publicApi.addResourceWithCorsOptionsSubTree(testApiResource,"v1");
    const testResource1 = v1Resource.addResource("test1");
    const testResource2 = v1Resource.addResource("test2");

    const testGetIntegration1 = new DigitrafficIntegration(testGetLambda, MediaType.TEXT_PLAIN)
        .passAllQueryParameters()        
        .addMultiValueQueryParameter("multi")
        .build();

    const testGetIntegration2 = new DigitrafficIntegration(testGetLambda, MediaType.TEXT_PLAIN)
