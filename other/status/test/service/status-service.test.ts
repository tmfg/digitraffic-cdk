import * as StatusService from "../../lib/service/status-service";
import {
    NODEPING_DIGITRAFFIC_USER,
    NodePingApi,
    NodePingCheck,
    NodePingContact
} from "../../lib/api/nodeping-api";
import {
    StatuspageApi,
    StatuspageComponent,
...
import { EndpointHttpMethod, MonitoredApp, MonitoredEndpoint } from "../../lib/app-props";
import { emptySecretHolder, mockSecretManager, setTestEnv } from "../testutils";
import {
    CStateStatus,
    CStateStatuspageApi,
    CStateSystem,
    PinnedIssue
} from "../../lib/api/cstate-statuspage-api";
import { AppWithEndpoints } from "../../lib/model/app-with-endpoints";
import { DigitrafficApi } from "../../lib/api/digitraffic-api";
...
describe("StatusServiceTest", () => {
    beforeAll(() => {
        setTestEnv();
        mockSecretManager();
        secretHolder = SecretHolder.create<UpdateStatusSecret>();
    ...
    test("updateChecks - check is updated", async () => {
        ...
        await StatusService.updateChecks(
            checks,
            [Object.keys(slackContact.addresses)[0]],
            "gh-contact-id",
    ...
    test("updateComponentsAndChecksForApp - missing check -> new check is created", async () => {
        ...
        checks[0].notifications.push(
            ...
            { [Object.keys(check_0_contact.addresses)[0]]: { delay: 0, schedule: "All" } },
            { [Object.keys(ghActionsContact.addresses)[0]]: { delay: 0, schedule: "All" } }
        );
        ...
        const cStateStatus = {
            systems: [makeCStateSystem("Road", endpoints[0]), makeCStateSystem("Road", endpoints[1])],
            pinnedIssues: []
        } satisfies CStateStatus;
...
async function callWithStubsAndVerifyUpdateComponentsAndChecksForApp(
    ...
    // const getAppEndpointsSpy = jest
    //     .spyOn(digitrafficApi, "getAppEndpoints")
...
function makeCStateSystem(category: "Road" | "Marine" | "Rail", url: string): CStateSystem {
    return {
        name: `${category.toLowerCase()}${url}`,
        status: "ok",
        category: category,
        description: `Api for ${category} ${url}`
    };
}
