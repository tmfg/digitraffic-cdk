...
describe("status service", () => {
    function testGetNodePingAndStatuspageComponentStatuses(
        ...
        test(name, async () => {
            ...
            sinon
                .stub(statuspageApi, "getStatuspageComponents")
                .returns(Promise.resolve(returnFromStatuspage));
            sinon
                .stub(nodePingApi, "getNodepingChecks")
                .returns(Promise.resolve(returnFromNodePing));
    ...
    testGetNodePingAndStatuspageComponentStatuses(
        "getNodePingAndStatuspageComponentStatuses - returns nothing",
        (statuses: string[]) => expect(statuses.length).toBe(0)
    );

    testGetNodePingAndStatuspageComponentStatuses(
        "getNodePingAndStatuspageComponentStatuses - missing Statuspage component",
        (statuses: string[]) => {
    ...
    testGetNodePingAndStatuspageComponentStatuses(
        "getNodePingAndStatuspageComponentStatuses - missing NodePing check",
        (statuses: string[]) => {
            expect(statuses.length).toBe(1);
    ...
    testGetNodePingAndStatuspageComponentStatuses(
        `getNodePingAndStatuspageComponentStatuses - component groups don't create checks`,
        (statuses: string[]) => {
    ...
    testGetNodePingAndStatuspageComponentStatuses(
        `getNodePingAndStatuspageComponentStatuses - NodePing check UP, Statuspage check DOWN`,
        (statuses: string[]) => {
            expect(statuses.length).toBe(1);
            expect(statuses[0]).toBe(
                "testcomponent: NodePing check is UP, Statuspage component is DOWN"
            );
        },
        [
            {
    ...
    testGetNodePingAndStatuspageComponentStatuses(
        `getNodePingAndStatuspageComponentStatuses - NodePing check DOWN, Statuspage check UP`,
        (statuses: string[]) => {
            expect(statuses.length).toBe(1);
            expect(statuses[0]).toBe(
                "testcomponent: NodePing check is DOWN, Statuspage component is UP"
            );
        },
        [
            {
    ...
    testGetNodePingAndStatuspageComponentStatuses(
        `getNodePingAndStatuspageComponentStatuses - app name is stripped`,
        (statuses: string[]) => {
            expect(statuses.length).toBe(0);
        },
        [
    ...
    test("updateChecks - check is updated ", async () => {
        ...
        const nodePingApiUpdateSpy = sinon
            .stub(nodePingApi, "updateNodepingCheck")
