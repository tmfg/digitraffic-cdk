...
export async function handleMaintenance(
  ...
  if (activeMaintenance && enabledChecksCount) {
    ...
    const disabledChecksAfterEnable =
      nodePingApi.getDisabledNodePingChecks(checksAfterEnable);
    ...
    logger.info({
      method: method,
      message: `No change in maintenance status, activeMaintenance: ${JSON.stringify(
        activeMaintenance,
      )}`,
    });
  }
