...
import { getActiveMaintenance, getCstateIndexJson, mockSecretHolder, setTestEnv } from "../testutils.js";
...
describe("CStateApiTest", () => {
    ...
    test("findActiveMaintenance - active maintenance, not with NodePing disable", async (): Promise<void> => {
        await expectFindActiveMaintenance([{ disableNodeping: false, start: getDateInPast() }], false);
    });

    test("findActiveMaintenance - active maintenance, with NodePing disable", async (): Promise<void> => {
        await expectFindActiveMaintenance([{ disableNodeping: true, start: getDateInPast() }], true);
    });

    test("findActiveMaintenance - future maintenance, not with NodePing disable", async (): Promise<void> => {
        await expectFindActiveMaintenance([{ disableNodeping: false, start: getDateInFuture() }], false);
    });

    test("findActiveMaintenance - future maintenance, with NodePing disable", async (): Promise<void> => {
        await expectFindActiveMaintenance([{ disableNodeping: true, start: getDateInFuture() }], false);
    });

    test("findActiveMaintenance - future maintenance in 1 minute, with NodePing disable should activate", async () => {
        await expectFindActiveMaintenance([{ disableNodeping: true, start: getDateInFuture(30) }], true);
    });
    ...
    async function expectMaintenance(
        ...
        const spy = jest
            ...
            .mockImplementation((_url: string, _config?: AxiosRequestConfig<unknown>): Promise<unknown> => {
                expect(_url).toEqual(`${getEnvVariable(StatusEnvKeys.C_STATE_PAGE_URL)}/index.json`);
    ...
    async function expectTriggerUpdateMaintenanceGithubAction(maintenance: ActiveMaintenance): Promise<void> {
        const url =
            `https://api.github.com/repos/${getEnvVariable(StatusEnvKeys.GITHUB_OWNER)}/${getEnvVariable(StatusEnvKeys.GITHUB_REPO)}/actions/workflows/${getEnvVariable(StatusEnvKeys.GITHUB_UPDATE_MAINTENANCE_WORKFLOW_FILE)}/dispatches` as const;
        const gitHubPat = (await secretHolder.get()).gitHubPat;
        const spy = jest
            .spyOn(ky, "post")
            .mockImplementation((_url: Input, _options?: Options | undefined): ResponsePromise => {
                expect(_url).toEqual(url);
                if (!_options) {
                    throw new Error("_options have to be defined");
                }
                const data = _options.json as GithubActionPostData satisfies GithubActionPostData;
                const headers = _options.headers as Record<string, string>;
                expect(data.ref).toEqual(`refs/heads/${getEnvVariable(StatusEnvKeys.GITHUB_BRANCH)}`);
                expect(data.inputs.baseUrl).toEqual(getEnvVariable(StatusEnvKeys.C_STATE_PAGE_URL));
                expect(data.inputs.permalink).toMatch(
                    `${getEnvVariable(StatusEnvKeys.C_STATE_PAGE_URL)}/issues/digitraffic-maintenance/`
                );

                // eslint-disable-next-line dot-notation
                expect(headers["Accept"]).toEqual("application/vnd.github+json");
                expect(headers["Content-Type"]).toEqual("application/vnd.github+json");
                // eslint-disable-next-line dot-notation
