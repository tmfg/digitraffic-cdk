...
import { NodePingApi, NodePingCheck, NodePingContact } from "../api/nodeping-api";
...
import { AppWithEndpoints } from "../model/app-with-endpoints";
import { UpdateStatusSecret } from "../secret";
import { SecretHolder } from "@digitraffic/common/dist/aws/runtime/secrets/secret-holder";
import { CStateStatuspageApi, CStateSystem } from "../api/cstate-statuspage-api";
import { removeAppAndTrim } from "./utils";
...
function convertToCstateNameWithoutApp(checkLabel: string): string {
    ...
    const filledLabel = fillSpacesAndTrim(labelWoApp);
...
function getContactIds(
    ...
    return isBeta(checkLabel)
        ...
        : contact && contact?.addresses && Object.keys(contact?.addresses).length
        ? [...internalContactIds, githubActionsContactId, Object.keys(contact.addresses)[0]]
...
function getContactId(contactName: string, contacts: NodePingContact[]): string {
    ...
    if (contact) {
        if (!contact.addresses || !Object.keys(contact.addresses).length) {
            logger.error({
                method,
        ...
        return Object.keys(contact.addresses)[0];
    } else {
        logger.error({
            method,
