...
export async function getNodePingAndStatuspageComponentStatuses(
    ...
    const statuspageComponents = (
        await statuspageApi.getStatuspageComponents()
    ...
    const missingNodePingChecks = statuspageComponents
        .filter((sc) => !(sc.name in nodePingCheckMap))
        .map((sc) => `${sc.name}: NodePing check missing`);
    const otherErrors = statuspageComponents
        .map((sc) => {
...
function getStatuspageComponentGroupId(
...
async function updateComponentsAndChecksForApp(
    ...
    const allEndpoints = appEndpoints.endpoints.concat(
        appEndpoints.extraEndpoints.map((e) => e.name)
    );

    let statuspageComponents = await statuspageApi.getStatuspageComponents();
    const statuspageComponentNames: string[] = statuspageComponents.map(
        (c) => c.name
    );
    ...
    const contactNames: string[] = Object.values(contacts).map(
        (c: any) => c.name
    );
    const missingContacts = allEndpoints.filter(
        (e) =>
            !contactNames.includes(e) &&
            !contactNames.includes(`${appEndpoints.app} ${e}`)
    );
    console.log("Missing contacts", missingContacts);

    for (const missingContact of missingContacts) {
        const component = statuspageComponents.find(
            (c) => c.name === missingContact
        );
        if (!component) {
            throw new Error(
                `Component for missing contact ${missingContact} not found`
            );
        }
    ...
    const missingChecks = allEndpoints.filter(
        (e) =>
            ...
            !checkNames.includes(`${appEndpoints.app} ${e}`)
    );
    console.log("Missing checks", missingChecks);

    for (const missingCheck of missingChecks) {
        const contact: any = Object.values(contacts).find(
            (c: any) =>
                c.name === missingCheck ||
                `${appEndpoints.app} ${c.name}` === missingCheck ||
                c.name === `${appEndpoints.app} ${missingCheck}`
        );
        if (!contact) {
            throw new Error(`Contact for ${missingCheck} not found`);
        }
        const correspondingExtraEndpoint = appEndpoints.extraEndpoints.find(
    ...
    await updateChecks(
        checks.filter((c) =>
            c.label.toLowerCase().includes(appEndpoints.app.toLowerCase())
        ),
        nodePingApi,
        appEndpoints.extraEndpoints
    );
}
...
export async function updateChecks(
    ...
    for (const check of checks) {
        const correspondingExtraEndpoint = extraEndpoints.find(
            (e) => e.url === check.parameters.target
        );
...
export async function updateComponentsAndChecks(
    ...
    const endpoints: AppEndpoints[] = await Promise.all(
        apps.map(digitrafficApi.getAppEndpoints)
    );
