...
def make_adjustments(df):
    ...
    for name, data_points in missing_figures.items():
        for timestamp, new_road_value in data_points.items():
            filter = (df['aikaleima'] == timestamp) & (df['request_uri'] == '') & (df['name'] == name)
            old_road_value = df.loc[filter & (df['liikennemuoto'] == 'tie')]['value'].item()
            traffic_combined_value = df.loc[filter & (df['liikennemuoto'] == 'kaikki')]['value'].item()

            # korjaa yksittäiset
            df.loc[filter & (df['liikennemuoto'] == 'tie'), ['value']] = new_road_value

            # korjaa yhteensä arvo
            df.loc[filter & (df['liikennemuoto'] == 'kaikki'), ['value']] = traffic_combined_value - old_road_value + new_road_value
...
def fetch_data_from_database(logger):
    ...
    url = 'mysql+pymysql://{username}:{password}@{host}:{port}/{database}'.format(**secret)
    engine = create_engine(url)

    with engine.connect() as conn:
        df = pd.read_sql_query(SQL_QUERY, conn,
                               index_col='id', parse_dates=['from', 'to'])

    logger.info(f'method=data.fetch_data_from_database took={time.time()-start} mesasage=read data from database')
...
def create_dataframe(logger=None):
    ...
    df.loc[df['name'].isin(has_integer_values), ['value']] = df[df['name'].isin(has_integer_values)]['value'].apply(np.uint)
