...
logger = Logger(service="data-dump-compositions")
...
def lambda_handler(event, context):
    ...
    logger.info('Composition archiving complete')
...
def uploadToS3(zipPath, month_start: datetime.date):
    s3_filename = f'digitraffic-rata-compositions-{month_start}.zip'
...
def createZipFile(start_date: datetime.date, end_date: datetime.date):
    ...
    zipPath = f'/tmp/digitraffic-rata-compositions-{start_date}.zip'

    logger.info('Starting composition archiving', start_date=str(start_date), end_date=str(end_date))
    ...
    with zipfile.ZipFile(zipPath, 'w', zipfile.ZIP_DEFLATED) as zipped_f:
        ...
        while start_date < end_date:
            ...
            logger.info('Fetching compositions', url=url, file_name=filename)

            compositions = requests.get(url, headers=HEADERS)
            compositions.raise_for_status()
            logger.info('Compositions response', status=compositions.status_code, length=len(compositions.content), date=str(start_date))
