...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "dump-compositions"))
dump_compositions = importlib.import_module("dump-compositions")

BUCKET_NAME = "test-bucket"
REGION = "eu-west-1"

MOCK_RESPONSE = [{"trainNumber": 1}]


class TestDumpCompositions(unittest.TestCase):
    ...
    def test_eventbridge_trigger_uses_previous_month(self):
        ...
        Given: Today is 2026-03-15 (previous month is February 2026 with 28 days),
               ...
               mock API returns composition data for each day,
        ...
        Then:  28 API calls are made (one per day of February),
               one zip is uploaded to S3 with key 'digitraffic-rata-compositions-2026-02-01.zip'
        """
        # Given
        ...
        with mock_aws():
            ...
            with rm.Mocker() as m:
                ...
                with patch.object(dump_compositions.datetime, "date") as mock_date:
                    mock_date.today.return_value = pinned_today
                    mock_date.side_effect = lambda *args, **kwargs: date(*args, **kwargs)

                    with patch.dict(os.environ, {"DUMP_BUCKET_NAME": BUCKET_NAME}):
                        # When
                        result = dump_compositions.lambda_handler(event, context)
            ...
            self.assertEqual(
                ...
                "https://rata.digitraffic.fi/api/v1/compositions/2026-02-01",
            )
            self.assertEqual(
                m.request_history[-1].url,
                "https://rata.digitraffic.fi/api/v1/compositions/2026-02-28",
            )

            objects = s3.list_objects_v2(Bucket=BUCKET_NAME)
            key = objects["Contents"][0]["Key"]
            self.assertEqual(key, "digitraffic-rata-compositions-2026-02-01.zip")
    ...
    def test_manual_trigger_with_month_parameter(self):
        ...
        Then:  31 API calls are made (one per day of December),
               one zip is uploaded to S3 with key 'digitraffic-rata-compositions-2025-12-01.zip',
               zip contains 31 JSON files named '{date}_compositions.json'
        """
        # Given
        context = MagicMock(spec=LambdaContext)
        event = {"month": "2025-12"}
        ...
        with mock_aws():
            ...
            self.assertEqual(
                m.request_history[0].url,
                "https://rata.digitraffic.fi/api/v1/compositions/2025-12-01",
            )
            self.assertEqual(
                m.request_history[-1].url,
                "https://rata.digitraffic.fi/api/v1/compositions/2025-12-31",
            )
            ...
            key = objects["Contents"][0]["Key"]
            self.assertEqual(key, "digitraffic-rata-compositions-2025-12-01.zip")
            ...
            with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
                ...
                self.assertEqual(filenames[0], "2025-12-01_compositions.json")
                self.assertEqual(filenames[-1], "2025-12-31_compositions.json")
            ...
            with rm.Mocker() as m:
                ...
                with patch.dict(os.environ, {"DUMP_BUCKET_NAME": BUCKET_NAME}):
                    ...
                    with self.assertRaises(requests.HTTPError):
                        dump_compositions.lambda_handler(event, context)
