import { type DbRamiMessage, findActiveMessages, findMessagesUpdatedAfter } from "../dao/message.js";
        ...
        ...(messageContainsAudio(message) && {
            audio: {
                ...
                ...(audioContainsDeliveryRules(message) && {
                    deliveryRules: {
                        startDateTime: message.audio?.delivery_rules?.start_date ?? undefined,
                        ...
                        repeatEvery: message.audio?.delivery_rules?.repeat_every ?? undefined,
        ...
        ...(messageContainsVideo(message) && {
            video: {
                ...
                ...(videoContainsDeliveryRules(message) && {
                    deliveryRules: {
                        startDateTime: message.video?.delivery_rules.start_date ?? undefined,
...
function messageContainsAudio(message: DbRamiMessage): boolean {
    return !!message.audio.text_fi || !!message.audio.text_en || !!message.audio.text_sv;
}
...
function messageContainsVideo(message: DbRamiMessage): boolean {
    return !!message.video.text_fi || !!message.video.text_en || !!message.video.text_sv;
}
