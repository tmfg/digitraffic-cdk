...
describe(
  ...
  dbTestBase(() => {
    test("insertMessage - insert valid DtRamiMessage", async () => {
      ...
      expect(result[0]?.stations?.split(",").sort()).toEqual(
        message?.stations?.sort(),
      );
      expect(
        mapBitsToDays(
    ...
    test("findMessagesUpdatedAfter - correct", async () => {
      ...
      const messageBeforeDate = createDtRamiMessage({
        id: "abc",
        created: subDays(date, 1),
      });
    ...
    test("findMessagesUpdatedAfter - inactive messages returned if parameter set", async () => {
      ...
      const result = await findMessagesUpdatedAfter(
        date,
        null,
        null,
        null,
