  ...
  dbTestBase(() => {
    test("handler - valid parameters", async () => {
      const result = await handler(
        createGetMessagesUpdatedAfterLambdaEvent({ date: "2023-01-01" }),
      );
      expect(result.status).toEqual(200);
    });
    test("handler - invalid parameters", async () => {
      const result = await handler(
        createGetMessagesUpdatedAfterLambdaEvent({ date: "abc" }),
      );
      expect(result.status).toEqual(400);
    });
    test("handler - date parsing", () => {
      const dateString = "2023-01-01T12:00Z";
      const lambdaEventMessage = createGetMessagesUpdatedAfterLambdaEvent({
        date: dateString,
      });
