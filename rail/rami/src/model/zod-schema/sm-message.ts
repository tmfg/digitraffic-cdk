...
const stopAssignment = z.object({
  expectedQuayName: z.string().describe("expected quay for this stop")
    .optional(),
});
...
export const monitoredCall = z.object({
  ...
  aimedArrivalTime: z.string().describe(
    "datetime, in UTC ISO8601, expected arrival time",
  ).optional(),
  aimedDepartureTime: z.string().describe(
    "datetime, in UTC ISO8601, expected arrival time",
  ).optional(),
  expectedArrivalTime: z.string().describe(
    "datetime, in UTC ISO8601, expected arrival time",
  ).optional(),
  expectedDepartureTime: z.string().describe(
    "datetime, in UTC ISO8601, expected arrival time",
  ).optional(),
