...
import {
  parseUDOTMessage,
  saveSMMessage,
} from "../../service/process-sm-message.js";
      ...
      return await Promise.allSettled(event.Records.map(async (r) => {
        ...
        try {
          ...
          if (udotMessage) {
            ...
            await saveSMMessage(
              udotMessage.messageId,
              udotMessage.trainNumber,
              udotMessage.departureDate,
              recordBody,
            );
            ...
            if (udotMessage.vehicleJourneyName.includes("BUS")) {
              logger.info({
                method: "RAMI-ProcessSmQueue.handler",
                message:
                  `Skipping message ${udotMessage.messageId} with vehicleJourneyName ${udotMessage.vehicleJourneyName}`,
